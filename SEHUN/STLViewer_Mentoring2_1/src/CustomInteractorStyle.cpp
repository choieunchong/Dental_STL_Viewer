	...
	if (cellPicker->GetCellId() != -1)
	{
			...
			// 찍은 면의 버텍스(3개)만큼 for문 돌림
		{
			...
			else if (flag == 1)
			{
				if (min > distance)
				{
					vertexIdxs.push_back(fv_iter->idx());
					minVec = point;
				}
			}
			flag = 1;
		...
		if (pickCount > 1)
		{
			//if (lastPickCount != pickCount)							// 피킹 여러번 했을 때 초기화 시켜줌
			//{
			//	size = 0;
			//	overCount = 0;
			//	lastSize = 1;
			...
			//{
			//	if (shortPointHash[pickCount - 2][0] == triMesh.point(*v_it)[0] && shortPointHash[pickCount - 2][1] == triMesh.point(*v_it)[1] && shortPointHash[pickCount - 2][2] == triMesh.point(*v_it)[2])
			//	{
			...
			//{
			//	if (triMesh.point(*vv_it) != shortPointHash[pickCount - 2])
			//	{
			//		idHash[size] = vv_it.handle().idx();                        // 피킹한 점의 주변점의 아이디
			//		overVector[size] = (triMesh.point(*vv_it));                 // 주변 벡터
			//		size++;                                                     // 해쉬의 전체 크기
			//		overCount++;                                                // 근접점 개수
			//		if (triMesh.point(*vv_it)[0] == shortPointHash[pickCount - 1][0] && triMesh.point(*vv_it)[1] == shortPointHash[pickCount - 1][1] && triMesh.point(*vv_it)[2] == shortPointHash[pickCount - 1][2])
			//		{
			//			location = 0;
			//			findLocation = true;
			//			break;
			...
			//int j = 1;													// 다익스트라 구하기 위한 배열 만들기
			//for (int i = 0; i <= location; i++)
			//{
			//	for (int count = 1; count <= nodeCount[idHash[i]]; count++)
			//	{
			...
			//		if (vertexHash[idHash[i]][j][0] == shortPointHash[pickCount - 1][0] && vertexHash[idHash[i]][j][1] == shortPointHash[pickCount - 1][1] && vertexHash[idHash[i]][j][2] == shortPointHash[pickCount - 1][2])
			//			break;
			//		j++;
			//	}
			//}
			std::vector<int> dijkstarPath = dijkstra(vertexIdxs[vertexIdxs.size() - 2], vertexIdxs.back(), triMesh);
			//dijkstra(idHash[0]);
